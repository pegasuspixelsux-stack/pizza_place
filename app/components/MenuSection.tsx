...
function CategoryBlock({ category }: { category: MenuCategory }) {
  return (
    <div>
      <h3 className="text-2xl font-semibold tracking-tight text-orange-50">
        {category.name}
      </h3>
          ...
          <div key={item.id} className="flex items-baseline justify-between gap-4">
            <div className="min-w-0">
              <div className="flex flex-wrap items-baseline gap-x-2">
                <h4 className="font-medium text-orange-50">{item.name}</h4>
                {item.tags && item.tags.length > 0 && (
                  <span className="flex flex-wrap gap-1.5">
                    {item.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-rusty-spice-600 px-2 py-0.5 text-[0.6875rem] font-medium tracking-wide text-rusty-spice-200"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <p className="mt-1.5 text-sm leading-relaxed text-rusty-spice-200">
                {item.description}
              </p>
            </div>
            <span className="shrink-0 font-medium tabular-nums text-orange-50">
